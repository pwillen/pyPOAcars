...
@dataclass
class FrameControlCode:
    hex_str: str
    bin_str: str
    name: str

    def __iter__(self):
        return iter((self.hex_str, self.bin_str, self.name))



def parse_acars_message(message: bytearray) -> str:
    """
...
def truncate_acars_message(message: bytearray) -> bytearray:
    """
...
def detect_acars_codes(demod_message: bytearray) -> list[FrameControlCode]:
    """
    A function to analyze the bytes in a demodulated ACARS message to detect expected control characters
    :param demod_message: The demodulated ACARS message
    Returns: A list of Control Codes in binary, hex, and code name
    """

    detected_codes = []
    for byte in demod_message:
        # Collect hex byte representation of the current byte
        hex_str = format(byte, "02x").upper()
        # Collect binary byte representation of the current byte
        binary_str = format(byte, "08b")

        if byte in EXPECTED_FRAME_CODES.values():
            # Reverse Key Lookup
            name = list(EXPECTED_FRAME_CODES.keys())[list(EXPECTED_FRAME_CODES.values()).index(byte)]

            # Append only if byte corresponds to a Control Code
            detected_codes.append(FrameControlCode(hex_str, binary_str, name))

    return detected_codes


def _synchronize_and_extract_bits(
    ...
    sample_index = next(
        index
        for index, value in enumerate(fh_signal[skip_index:].real, start=skip_index)
        if value <= 0.5 * fh_peak
    )
    ...
    bits = np.zeros(
        (len(fh_signal) - sample_index) // samples_per_symbol + 1, dtype=int
    )
    ...
    while sample_index < len(fh_signal) - 2 * samples_per_symbol:
        ...
        bits[bit_index] = (
            1 if fh_signal[sample_index].real > fl_signal[sample_index].real else 0
        )
        bit_index += 1

        # Freq Synchronization based on FH signal
        sample_index = _synchronize_signal(
            fh_signal,
            fl_signal,
...
def _synchronize_signal(
    ...
    if (
        main_signal[sample_index].real > alt_signal[sample_index].real
        and alt_signal[sample_index + samples_per_symbol].real
        > main_signal[sample_index + samples_per_symbol].real
        and alt_signal[sample_index - samples_per_symbol].real
        > main_signal[sample_index - samples_per_symbol].real
    ):
        ...
        for num_samples in range(
            -clock_deviation + 1, clock_deviation + 1
        ):  # -4 to 5 inclusive
...
def _nrzi_decode(bits_in: np.ndarray[int]) -> np.ndarray[int]:
    """
...
def _pack_acars_bytes(bits_in: np.ndarray[int]) -> bytearray:
    """
...
def demod(
    ...
    bits = _synchronize_and_extract_bits(
        fh_signal=fh_signal,
        fl_signal=fl_signal,
    ...
    nrzi_decoded = _nrzi_decode(bits)

    # Pack the bits into bytes being conscious of the 8th bit being a parity bit
    demod_message = _pack_acars_bytes(nrzi_decoded)

    # This is more for debugging and visualization
    detected_codes = detect_acars_codes(demod_message)
...
def message_region_detection(
    ...
    smoothed_region = np.convolve(
        message_region.astype(int), np.ones(window_size), mode="same"
    )
